...
def load_both_stitching(pos, neg, auto_factor=False):
    '''
    Load images for a double helicity reconstruction
    INPUT:  pos, neg: arrays, images of positive and negative helicity
            auto_factor: optional, boolean, determine the factor by which neg is multiplied automatically, if FALSE: factor is set to 0.5 (defualt is False)
    OUTPUT: difference hologram,
            factor,
            offset pos and neg for stitching
    --------
    author: RB 2020
    '''
    size = pos.shape
    ...
    #make sure to return a quadratic image, otherwise the fft will distort the image
    if size[0]<size[1]:
        return (pos[:, :size[0]] - factor * topo[:, :size[0]], factor,offset_pos,offset_neg)
    elif size[0]>size[1]:
        return (pos[:size[1], :] - factor * topo[:size[1], :], factor,offset_pos,offset_neg)
    else:
        return (pos - factor * topo, factor,offset_pos,offset_neg)
